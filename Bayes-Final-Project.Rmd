...
x[, 1] <- rnorm(N, 0, sx) #Assume a normal prior 

weights[, 1] <- dnorm(obs[1], x[, 1], sy) # initial weight set
...
for (t in seq(2, T)) {
  ...
  x[, t] <- rnorm(N, x[, t - 1], sx) #sample x from previous x

  weights[, t] <- dnorm(obs[t], x[, t], sy) #get weights from obsveration
